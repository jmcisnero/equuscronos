...
export default function LeaderboardTable({
  ...
  // Formateador robusto de tiempos de carrera en formato humano (hh h mm m ss s)
  ...
  const renderStatusBadge = (status: string) => {
    switch (status) {
      case "IN_RACE":
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-950 border border-emerald-300 animate-pulse">
      ...
      case "VET_CHECK":
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-amber-50 text-amber-950 border border-amber-300 animate-pulse">
      ...
      case "RESTING":
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-sky-50 text-sky-950 border border-sky-300">
            <span className="h-1.5 w-1.5 rounded-full bg-sky-600 mr-1.5"></span>
            Neutralizado
          </span>
        );
      case "FINISHED":
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-slate-900 text-white border border-slate-700">
            Finalizado
          </span>
        );
      case "DQ":
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-rose-50 text-rose-950 border border-rose-300">
            Descalificado (DQ)
          </span>
        );
      case "DNF":
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-slate-100 text-slate-800 border border-slate-300">
            Retirado (DNF)
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center px-2.5 py-1.5 rounded-full text-xs font-bold bg-gray-100 text-gray-800">
            {status}
          </span>
        );
    }
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-100 text-xs">
                  ...
                  <div>
                    ...
                    <span className="font-mono font-bold text-slate-900">{formatGap(entry.gapToLeaderMs)}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block font-bold text-[9px] uppercase tracking-wider">Velocidad Promedio</span>
                    <span className="font-extrabold text-slate-800">
                      {entry.averageSpeed ? `${entry.averageSpeed.toFixed(3)} km/h` : "--"}
                    </span>
                  </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 text-xs font-bold uppercase tracking-wider">
                <th className="py-4.5 px-6 text-center w-20">Puesto</th>
                <th className="py-4.5 px-6 text-center w-24">Dorsal</th>
                <th className="py-4.5 px-6">Binomio (Jinete / Caballo)</th>
                <th className="py-4.5 px-6 text-center">Etapa</th>
                <th className="py-4.5 px-6">Tiempo Neto</th>
                <th className="py-4.5 px-6">Diferencia</th>
                <th className="py-4.5 px-6">Velocidad Prom.</th>
                <th className="py-4.5 px-6 text-center">Pulso</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
              {filteredData.length > 0 ? (
                    ...
                    >
                      ...
                      <td className="py-4.5 px-6">
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-slate-500 font-bold flex items-center mt-0.5">
                      ...
                      {/* DIFERENCIA */}
                      <td className="py-4.5 px-6 font-mono text-xs text-slate-700">
                      ...
                      <td className="py-4.5 px-6 text-center">
                        {entry.heartRate ? (
                          <span className={`inline-block font-mono font-extrabold text-xs px-2 py-1 rounded-lg ${
                            entry.heartRate > 64
                              ? "bg-rose-100 text-rose-900 border border-rose-200"
                              : "bg-slate-100 text-slate-900 border border-slate-200"
                          }`}>
                ...
                <tr>
                  <td colSpan={9} className="py-12 px-6 text-center text-slate-500 font-medium">
                    No se encontraron binomios que coincidan con la búsqueda.
                  </td>
                </tr>
              )}
