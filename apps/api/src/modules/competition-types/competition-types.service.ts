...
export class CompetitionTypesService {
  ...
  async create(
    createCompetitionTypeDto: CreateCompetitionTypeDto,
  ): Promise<CompetitionType> {
    const existing = await this.compTypeRepository.findOne({
      where: { name: createCompetitionTypeDto.name },
    });
    if (existing) {
      throw new ConflictException(
        `La categoría '${createCompetitionTypeDto.name}' ya existe.`,
      );
    }
  ...
  async findOne(id: string): Promise<CompetitionType> {
    ...
    if (!type)
      throw new NotFoundException(
        `Tipo de competencia con ID ${id} no encontrado.`,
      );
    return type;
  }
  ...
  async update(
    id: string,
    updateCompetitionTypeDto: UpdateCompetitionTypeDto,
  ): Promise<CompetitionType> {
    const type = await this.findOne(id);
    if (
      updateCompetitionTypeDto.name &&
      updateCompetitionTypeDto.name !== type.name
    ) {
      const existing = await this.compTypeRepository.findOne({
        where: { name: updateCompetitionTypeDto.name },
      });
      if (existing) {
        throw new ConflictException(
          `La categoría '${updateCompetitionTypeDto.name}' ya existe.`,
        );
      }
    }
