...
export class CreateHorseDto {
  ...
  @IsString({ message: 'El identificador del propietario debe ser un texto válido.' })
  @IsNotEmpty({ message: 'El propietario del caballo es obligatorio para garantizar la trazabilidad de los binomios.' })
  ...
  @IsOptional()
  @IsString({ message: 'El código de microchip RFID debe ser un texto válido.' })
  chipId?: string;

  @ApiPropertyOptional({ description: 'Indica si tiene los controles sanitarios (AIE/Glándermo) al día', default: true })
  @IsOptional()
  @IsBoolean({ message: 'El estado de habilitación FEU debe ser un valor booleano.' })
  isFeuActive?: boolean;

  @ApiPropertyOptional({ description: 'Fecha de vencimiento de sanidad', example: '2026-12-31' })
  @IsOptional()
  @IsDateString({}, { message: 'La fecha de vencimiento de sanidad ingresada no es válida. Use el formato YYYY-MM-DD.' })
