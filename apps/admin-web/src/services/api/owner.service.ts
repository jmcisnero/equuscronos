...
export const OwnerService = {
  ...
  async getAll(search?: string): Promise<Owner[]> {
    const url = search
      ? `${API_URL}/owners?search=${encodeURIComponent(search)}`
      : `${API_URL}/owners`;
    const response = await fetch(url);
    if (!response.ok)
      throw new Error("Error al cargar la lista de propietarios");
  ...
  async getById(id: string): Promise<Owner> {
    ...
    if (!response.ok)
      throw new Error("Error al obtener el propietario especificado");
    return response.json();
  },
  ...
  async create(dtoOrName: CreateOwnerDto | string): Promise<Owner> {
    const payload =
      typeof dtoOrName === "string" ? { name: dtoOrName } : dtoOrName;
