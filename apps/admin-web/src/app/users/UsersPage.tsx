...
export function UsersPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setError(err.message || 'Error al cargar los registros del staff y organizaciones.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const filteredUsers = users.filter((u) =>
    ...
    (u.tenant && u.tenant.name.toLowerCase().includes(searchQuery.toLowerCase()))
  );

  const filteredTenantsForSelect = tenants.filter((t) =>
    t.name.toLowerCase().includes(tenantFilterQuery.toLowerCase()) ||
    (t.location && t.location.toLowerCase().includes(tenantFilterQuery.toLowerCase()))
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.trim() || !email.includes('@')) {
      setFormError('Por favor, introduzca una dirección de correo electrónico válida.');
      setIsSaving(false);
      return;
    }
  ...
  const handleDelete = async (id: string, userName: string) => {
    if (confirm(`¿Está seguro de que desea revocar el acceso y eliminar definitivamente al usuario "${userName}"?`)) {
      try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Gestión de Usuarios y Staff</h1>
          <p className="mt-1 text-sm text-slate-500">
            Administre cuentas oficiales de oficiales de carrera, comisiones de juzgamiento, cuerpo veterinario y operadores de control de paso.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="py-12 text-center text-rose-600 font-semibold p-6">
              ...
              <button onClick={loadData} className="text-xs text-equus-green underline font-bold">
                Reintentar cargar
              </button>
            </div>
            ...
            <div className="py-20 text-center text-slate-500 p-6">
              ...
              <p className="text-xs text-slate-400 mt-1">Registre oficiales o delegados técnicos para darles acceso a la plataforma.</p>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-slate-50/75 border-b border-gray-100">
                <tr>
                  <th scope="col" className="py-4 pl-6 pr-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nombre del Operador</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Correo Electrónico</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Rol de Privilegios</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Club / Organización Afiliada</th>
                ...
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="whitespace-nowrap px-4 py-4 text-sm font-medium">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold ${getRoleBadge(user.role)}`}>
                        {getRoleLabel(user.role)}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-4 py-4 text-sm font-semibold text-slate-700">
                      {user.tenant ? (
                        <span className="text-slate-800">{user.tenant.name}</span>
                      ) : (
                    ...
                    <td className="whitespace-nowrap px-4 py-4 text-sm text-slate-400">
                      {user.createdAt ? (
                        <span>{new Date(user.createdAt).toLocaleDateString('es-UY')}</span>
                      ) : (
                        <span>-</span>
                      )}
                    </td>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-100 animate-slide-up">
            <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-base font-extrabold text-slate-800">
                  {editingUser ? 'Modificar Usuario / Staff' : 'Registrar Nuevo Operador / Oficial'}
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">Control de Roles y Contexto Organizacional</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-visible">
              ...
              <div>
                ...
                >
                  <option value={UserRole.USER}>Operador de Campo (Básico)</option>
                  <option value={UserRole.TIMEKEEPER}>Cronometrador Oficial</option>
                  <option value={UserRole.VET}>Veterinario Homologado</option>
                  <option value={UserRole.JUDGE}>Juez General de Comisión</option>
                  <option value={UserRole.ADMIN}>Super Administrador del Sistema</option>
                </select>
              </div>
              ...
              <div className="relative">
                ...
                <div className="relative">
                  <input
                    ...
                    onBlur={() => setTimeout(() => setIsDropdownFocused(false), 200)}
                ...
                {isDropdownFocused && (
                  <div className="absolute z-10 w-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-lg max-h-48 overflow-y-auto divide-y divide-slate-50">
                    ...
                    {filteredTenantsForSelect.length === 0 ? (
                      <div className="px-4 py-2.5 text-xs text-slate-400 italic">No se encontraron clubes</div>
                    ) : (
                      filteredTenantsForSelect.map((t) => (
                        ...
                        >
                          ...
                          {t.location && <span className="text-[10px] text-slate-400 block">{t.location}</span>}
                        </div>
                      ))
                    )}
                  </div>
                ...
                <p className="mt-1 text-[10px] text-slate-400 leading-normal">
                  * Vincular al operador a un club garantiza que la seguridad RLS restrinja sus accesos exclusivamente a la base de datos de dicho club.
                </p>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={editingUser ? 'Dejar en blanco para mantener la actual' : 'Introduzca contraseña inicial'}
                  className="w-full px-3.5 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-equus-green/20 focus:border-equus-green text-slate-800 shadow-sm"
                />
              </div>
