...
function checkpointLabel(stage: Stage, type: "ARRIVAL" | "VET_IN"): string {
  const suffix = type === "ARRIVAL" ? "Llegada Meta" : "Presentación Veterinaria";
...
export default function ArribosContingenciaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!competitionId) { setErrorMsg("Debe seleccionar una competencia."); return; }
    if (!stageId) { setErrorMsg("Debe seleccionar un checkpoint / etapa."); return; }
    if (isNaN(bib) || bib <= 0) { setErrorMsg("El número de dorsal debe ser un entero positivo."); return; }
    if (!/^\d{2}:\d{2}:\d{2}$/.test(arrivalTime)) {
      setErrorMsg("El formato de hora debe ser HH:MM:SS."); return;
    }
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="bg-rose-50 border border-rose-200 rounded-2xl p-8 text-center max-w-md">
          ...
          <h2 className="text-lg font-bold text-rose-800 mb-2">Acceso Denegado</h2>
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Registro manual de tiempos de llegada en caso de falla de hardware en campo.
            Simula exactamente el comportamiento de la Field App.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 p-3.5 bg-amber-50 border border-amber-200/80 rounded-xl">
        ...
        <p className="text-xs font-semibold text-amber-800">
          MODO CONTINGENCIA · Los registros actualizan el clasificador en vivo en tiempo real (WebSocket). Verificar horario exacto antes de confirmar.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100/80 overflow-hidden">
        <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-slate-700">Ingreso de Tiempo Manual</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label htmlFor="competitionId" className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">
              Competencia Activa
            </label>
            {loadingComps ? (
          ...
          {/* Checkpoint / Stage */}
          <div>
            <label htmlFor="stageId" className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">
              Checkpoint / Etapa
            </label>
            {stages.length === 0 ? (
              <p className="text-sm text-slate-400 italic py-2">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2 mb-3">
                  {(["ARRIVAL", "VET_IN"] as const).map((rt) => (
                    ...
                    >
                      {rt === "ARRIVAL" ? "🏁 Llegada Meta" : "🩺 Presentación Vet."}
                    </button>
                  ))}
                </div>
          ...
          {/* Dorsal number */}
          <div>
            <label htmlFor="bibNumber" className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">
              Dorsal Nro.
            </label>
            <input
              id="bibNumber"
          ...
          {/* Time input */}
          <div>
            <label htmlFor="arrivalTime" className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">
            ...
            <p className="mt-1.5 text-[11px] text-slate-400">
              Pulse el ícono del reloj para capturar la hora actual del sistema. Edite manualmente si es necesario.
            </p>
          </div>
          ...
          {errorMsg && (
            <div className="flex items-start gap-3 p-3.5 bg-rose-50 border border-rose-200 rounded-xl">
              ...
              <div>
                <p className="text-xs font-bold text-rose-700">Error transaccional</p>
          ...
          {status === "success" && lastResult && (
            <div className="flex items-start gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${lastResult.eliminated ? "bg-rose-100" : "bg-emerald-100"}`}>
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-emerald-600 mt-0.5 font-mono">
                  ID: {lastResult.id.substring(0, 8)}… · {new Date(lastResult.recordedAt).toLocaleTimeString("es-UY")}
                </p>
          ...
          <button
            type="submit"
            disabled={status === "loading" || loadingComps || competitions.length === 0}
      ...
      <div className="text-center">
        <p className="text-[11px] text-slate-400">
          Operador: <strong className="text-slate-500">{user?.name}</strong> · Rol: <strong className="text-slate-500">{user?.role}</strong>
          {user?.tenantId && <> · Tenant: <strong className="text-slate-500">{user.tenantId.substring(0, 8)}…</strong></>}
        </p>
