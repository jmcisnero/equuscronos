...
export default function CompetitionsPage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      if (compTypesData.length > 0) {
        setFormData(prev => ({ ...prev, competitionTypeId: compTypesData[0].id }));
      }
    } catch (err: any) {
      console.error('Error al cargar datos iniciales de clubes y modalidades:', err);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('[SUBMIT ERROR] Error completo del servidor NestJS:', err.response || err);
      setFormError(err.message || 'Error al guardar la competencia en el servidor.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteCompetition = async (id: string, name: string) => {
    if (confirm(`¿Está seguro de que desea eliminar la competencia "${name}"? Se eliminarán todas las etapas asociadas.`)) {
      try {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Calendario de Competencias</h1>
          <p className="mt-1 text-sm text-slate-500">
            Planificación de raids federados, campeonatos nacionales de endurance y configuración de etapas Vet Gate.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="py-12 text-center text-rose-600 font-semibold">
              ...
              <button onClick={() => loadCompetitions(searchQuery)} className="text-xs text-equus-green underline font-bold">
                Reintentar cargar
              </button>
            </div>
            ...
            <div className="py-20 text-center text-slate-500">
              ...
              <p className="font-medium text-slate-700">No hay competencias registradas.</p>
              <p className="text-xs text-slate-400 mt-1">Cree una nueva competencia para empezar a gestionar cronometrajes y controles veterinarios.</p>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-slate-50/75 border-b border-gray-100">
                <tr>
                  <th scope="col" className="py-4 pl-6 pr-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nombre de Competencia</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Fecha</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Ubicación</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Etapas</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Distancia Total</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Límite Vet Gate</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Estado</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-4 py-4 text-sm font-semibold text-slate-700 font-mono">
                        {comp.stages ? `${comp.stages.length} etapas` : '0 etapas'}
                      </td>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5">
              ...
              <div className="border-t border-slate-100 pt-4 space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0">
                  <div>
                    <h4 className="text-sm font-extrabold text-slate-800">Fases y Etapas del Evento</h4>
                    <p className="text-[11px] text-slate-400 mt-0.5">Defina las distancias de carrera y tiempos de neutralización.</p>
                  </div>
                ...
                {formData.stages.length > 0 ? (
                  ...
                  <div className="text-center py-6 border-2 border-dashed border-slate-200 rounded-xl text-slate-400 text-xs">
                    No se han definido etapas aún. Use el autocompletado o agregue una etapa en el panel superior.
                  </div>
                )}
              </div>
