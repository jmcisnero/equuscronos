...
export function CompetitionTypesPage() {
  ...
  const loadCompetitionTypes = async () => {
    ...
    try {
      ...
      setError(err.message || 'Error al cargar las modalidades de competencia.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id: string, typeName: string) => {
    if (confirm(`¿Está seguro de que desea eliminar la modalidad "${typeName}"? Esta acción no se puede deshacer y fallará si hay competencias asociadas a ella.`)) {
      try {
        await CompetitionTypeService.delete(id);
        loadCompetitionTypes();
      } catch (err: any) {
        alert(err.message || 'No se pudo eliminar la modalidad debido a restricciones de integridad.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Reglas y Modalidades</h1>
          <p className="mt-1 text-sm text-slate-500">
            Configure plantillas de reglas de competencia (parámetros reglamentarios, límites de ritmo cardíaco y pesos mínimos) para automatizar la homologación.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="py-12 text-center text-rose-600 font-semibold bg-white rounded-2xl border border-slate-100/50 p-6 shadow-sm">
          ...
          <button onClick={loadCompetitionTypes} className="text-xs text-equus-green underline font-bold">
            Reintentar cargar
          </button>
        </div>
        ...
        <div className="py-20 text-center text-slate-500 bg-white rounded-2xl border border-slate-100/50 p-6 shadow-sm">
          ...
          <p className="font-medium text-slate-700">No se encontraron modalidades de competencia.</p>
          <p className="text-xs text-slate-400 mt-1">Cree una plantilla para comenzar a definir reglas de carrera.</p>
        </div>
      ) : (
              ...
              >
                <div>
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <h3 className="font-bold text-slate-800 text-lg leading-snug">{type.name}</h3>
                  ...
                  <div className="space-y-2 mb-6">
                    ...
                    {ruleKeys.length === 0 ? (
                      <span className="text-xs text-slate-400 italic">Sin reglas definidas. Todo manual.</span>
                    ) : (
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-auto">
                  <span className="text-[10px] text-slate-400 font-mono">
                    {type.createdAt ? `Creado: ${type.createdAt.substring(0, 10)}` : ''}
                  </span>
            ...
            <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-base font-extrabold text-slate-800">
                  {editingType ? 'Modificar Modalidad / Plantilla' : 'Registrar Nueva Modalidad'}
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">Control de Parámetros Reglamentarios</p>
              </div>
