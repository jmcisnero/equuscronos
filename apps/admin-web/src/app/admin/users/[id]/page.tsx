    ...
    return (
      <div className="p-6 max-w-2xl mx-auto text-center">
        <div className="bg-rose-50 border border-rose-100 rounded-2xl p-8 text-rose-600 font-semibold shadow-sm">
          <p className="text-lg mb-4">⚠️ {error || 'No se encontró el usuario solicitado.'}</p>
          <Link
            href="/users"
        ...
        <div className="bg-slate-50 border-b border-slate-100 p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-mono mt-0.5">ID Interno: {user.id}</p>
            </div>
          </div>
          <div>
            <span className={`inline-flex rounded-full px-4 py-1.5 text-xs font-bold shadow-sm ${getRoleBadge(user.role)}`}>
        ...
        <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between py-2 border-b border-slate-50">
              <span className="text-sm font-semibold text-slate-500">Correo Electrónico</span>
              <span className="text-sm font-bold text-slate-800">{user.email}</span>
            </div>

            <div className="flex justify-between py-2 border-b border-slate-50">
              <span className="text-sm font-semibold text-slate-500">Club / Organización</span>
              <span className="text-sm font-bold text-slate-800">
                {user.tenant ? (
                  <span className="text-slate-800 font-semibold">{user.tenant.name}</span>
                ) : (
            ...
            <div className="flex flex-col py-2 space-y-1">
              <span className="text-sm font-semibold text-slate-500">Alcance de Privilegios</span>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between py-2 border-b border-slate-50">
              <span className="text-sm font-semibold text-slate-500">Fecha Alta Cuenta</span>
              <span className="text-sm font-bold text-slate-800 font-mono">
                {user.createdAt ? new Date(user.createdAt).toLocaleDateString('es-UY') : '-'}
              </span>
            </div>

            <div className="flex justify-between py-2 border-b border-slate-50">
              <span className="text-sm font-semibold text-slate-500">Seguridad RLS (Multi-Tenant)</span>
              <span className="text-xs font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2.5 py-0.5 rounded-md">
                {user.tenant?.id ? 'Restringido a Tenant' : 'Filtro Desactivado (Global)'}
              </span>
            </div>
          </div>
