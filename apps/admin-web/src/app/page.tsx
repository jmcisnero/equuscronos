  ...
  const stats = [
    ...
    { 
      ...
      value: statsData?.activeCompetition 
        ? '1 ACTIVA' 
        : (statsData?.upcomingCompetitions?.filter(c => c.status === 'PLANNED').length.toString() || '0'), 
      change: statsData?.activeCompetition ? 'Carrera en curso' : 'Fase planificación', 
    ...
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-br from-equus-green to-emerald-950 rounded-2xl p-6 md:p-8 text-white shadow-xl border border-white/5 relative overflow-hidden">
        ...
        <div className="max-w-3xl relative z-10">
          ...
          <div className="mt-4 flex items-center space-x-2">
            ...
            <span className="text-xs font-bold text-slate-300 tracking-wider uppercase">Conectado a la base de datos nacional FEU</span>
          </div>
        </div>
      </div>
      ...
      {error && (
        <div className="p-4 bg-rose-50 border border-rose-100 rounded-2xl text-rose-700 text-sm font-semibold flex items-center space-x-2">
          ...
          <span>Error al sincronizar estadísticas en vivo: {(error as any).message || String(error)}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((item, idx) => {
          ...
          const isHighlight = item.name === 'Próximas Competencias' && statsData?.activeCompetition;
          ...
          return (
            ...
            >
              ...
              <div className="mt-4 flex items-baseline justify-between">
                <span className={`text-3xl font-extrabold tracking-tight ${isHighlight ? 'text-rose-600' : 'text-slate-950'}`}>
                  {item.value}
                </span>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-base font-bold text-slate-800 uppercase tracking-wider">Estado de Competencia</h3>
            ...
            {isLoading ? (
                ...
                <div className="relative z-10 flex flex-col justify-between h-full space-y-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-300 font-medium mt-1">
                        📍 {statsData.activeCompetition.location || 'Melo, Cerro Largo'} • 📅 {statsData.activeCompetition.competitionDate}
                      </p>
                    </div>

                    <div className="bg-white/10 border border-white/20 rounded-2xl px-4 py-2 text-center sm:text-right">
                      <span className="block text-[8px] text-slate-400 font-bold uppercase tracking-wider">Límite Vet Gate</span>
                  ...
                  <div className="border-t border-white/10 pt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <p className="text-xs text-slate-300 leading-relaxed max-w-md">
                      El cronómetro oficial y las estaciones de control veterinario se encuentran operativas en tiempo real. Habilite binomios e ingrese al motor de cronometraje.
                    </p>
              ...
              <div className="bg-white border border-slate-100 rounded-3xl p-5 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-slate-50 pb-3">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Calendario de Eventos</span>
                  <Link href="/competitions" className="text-xs font-bold text-equus-green hover:underline">
                    Ver todos
                  </Link>
                </div>
                
                {statsData?.upcomingCompetitions && statsData.upcomingCompetitions.length > 0 ? (
                  <div className="divide-y divide-slate-50">
                    {statsData.upcomingCompetitions.slice(0, 3).map((comp) => (
                      <div key={comp.id} className="py-3 flex items-center justify-between gap-4 group">
                        <div>
                          <h5 className="text-sm font-bold text-slate-800 group-hover:text-equus-green transition-colors">
                            {comp.name}
                          </h5>
                          <span className="text-[10px] text-slate-400 font-medium">
                            {comp.location || 'Cerro Largo'} • {comp.competitionDate}
                          </span>
                        </div>
                        <Link
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {quickLinks.map((item) => {
                ...
                const titleColorClass =
                  ...
                  item.color === 'tan' ? 'group-hover:text-amber-800' : 
          ...
          {isLoading ? (
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-5 space-y-4 shadow-sm">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-rose-700 leading-relaxed mt-1 font-medium">
                    {(statsData?.expiredHealthHorses || 0) > 0 ? (
                      <span>¡Atención! Hay **{statsData?.expiredHealthHorses} caballos** con vencimientos de sanidad expirada en el padrón nacional.</span>
                    ) : (
              ...
              <div className="space-y-3">
                ...
                {statsData?.expiringHorses && statsData.expiringHorses.length > 0 ? (
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`inline-flex items-center text-[9px] font-bold px-2 py-0.5 rounded ${
                            ...
                            {horse.healthRecordsExpiration ? horse.healthRecordsExpiration.substring(0, 10) : '-'}
                          </span>
                        </Link>
                      );
                    })}
              ...
              <div className="flex items-start space-x-3.5 p-4 rounded-xl bg-emerald-50/30 border border-emerald-100 hover:bg-emerald-50/60 transition-colors duration-200">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-emerald-800 leading-relaxed mt-1 font-medium font-sans">
                    Sincronización nacional realizada con éxito. {statsData?.activeHorses || 0} binomios habilitados para la próxima competencia oficial.
                  </p>
                </div>
              </div>
