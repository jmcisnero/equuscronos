...
export default function RidersPage() {
  ...
  const loadRiders = async (query?: string) => {
    ...
    try {
      ...
      setError(err.message || 'Ocurrió un error al cargar el padrón de jinetes.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEditModal = (rider: Rider) => {
    ...
    setFormData({
      ...
      medicalCardExpiration: rider.medicalCardExpiration ? rider.medicalCardExpiration.substring(0, 10) : ''
    });
    setFormError(null);
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nationalId.trim()) {
      setFormError('La Cédula de Identidad es obligatoria para el control de identidad.');
      setIsSaving(false);
      return;
    }
  ...
  const handleDeleteRider = async (id: string, name: string) => {
    if (confirm(`¿Está seguro de que desea eliminar al jinete "${name}"? Esta acción no se puede deshacer y fallará si el jinete ya registra pasadas en competencias.`)) {
      try {
        await RiderService.delete(id);
        loadRiders(searchQuery);
      } catch (err: any) {
        alert(err.message || 'No se pudo eliminar al jinete por restricciones de integridad referencial.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-2xl font-extrabold text-slate-800 tracking-tight">Padrón de Jinetes</h1>
          <p className="mt-1 text-sm text-slate-500">
            Control de licencias federativas y vigencia de carnets médicos de atletas en la Federación Ecuestre.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="py-12 text-center text-rose-600 font-semibold">
              ...
              <button onClick={() => loadRiders(searchQuery)} className="text-xs text-equus-green underline font-bold">
                Reintentar cargar
              </button>
            </div>
          ) : riders.length === 0 ? (
            <div className="py-20 text-center text-slate-500">
              ...
              <p className="font-medium text-slate-700">No se encontraron jinetes registrados.</p>
              <p className="text-xs text-slate-400 mt-1">Intente refinar los criterios de búsqueda o añada uno nuevo.</p>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-gray-100">
              <thead className="bg-slate-50/75 border-b border-gray-100">
                <tr>
                  <th scope="col" className="py-4 pl-6 pr-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nombre del Atleta</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Documento (CI)</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Nro. Licencia FEU</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Estado Licencia</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Fecha Nacimiento</th>
                  <th scope="col" className="px-4 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Ficha Médica</th>
                  ...
                  return (
                    <tr key={rider.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="whitespace-nowrap py-4 pl-6 pr-3 text-sm font-bold text-slate-900">
                        <Link href={`/admin/riders/${rider.id}`} className="text-equus-green hover:underline">
                          {rider.name}
                        </Link>
                      </td>
            ...
            <div className="bg-slate-50 border-b border-slate-100 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-base font-extrabold text-slate-800">
                  {editingRider ? 'Modificar Registro de Jinete' : 'Inscribir Nuevo Jinete'}
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">Padrón Oficial - Federación Ecuestre Uruguaya</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, feuId: e.target.value })}
              ...
              <div className="p-3 bg-slate-50 border border-slate-100 rounded-xl flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-slate-700">Estado de Licencia</span>
                  <span className="text-[10px] text-slate-400">¿El jinete está habilitado y al día con la anualidad?</span>
                </div>
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, isFeuActive: e.target.checked })}
