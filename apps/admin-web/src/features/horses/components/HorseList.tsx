...
export const HorseList: React.FC = () => {
  ...
  const fetchHorses = async (search?: string) => {
    ...
    try {
      ...
      setError(
        err.message || "Error al conectar con el servidor de EquusCronos.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDelete = async (id: string, name: string) => {
    if (
      !window.confirm(
        `¿Está seguro de que desea eliminar al caballo "${name}" del padrón? Esta acción no se puede deshacer y fallará si el equino ya registra pasadas en competencias.`,
      )
    )
      return;
    try {
      ...
      alert(
        err.message ||
          "No se pudo eliminar al caballo del padrón por restricciones de integridad referencial.",
      );
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Control de registros de equinos, chips oficiales RFID y estados
            sanitarios obligatorios de la FEU.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="py-12 text-center text-rose-600 font-semibold">
              ...
              <button
                onClick={() => fetchHorses(searchTerm)}
            ...
            <div className="py-20 text-center text-slate-500">
              ...
              <p className="font-medium text-slate-700">
                No se encontraron caballos registrados.
              </p>
              <p className="text-xs text-slate-400 mt-1">
                Asegúrese de escribir correctamente el nombre o chip.
              </p>
            </div>
          ) : (
